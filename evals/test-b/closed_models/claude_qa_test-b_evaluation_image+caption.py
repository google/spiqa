...
def prepare_inputs(paper, question_idx):
    ...
    for idx, figure in enumerate(all_figures_modified):
        ...
        all_figures_encoded['figure_{}'.format(idx)] = encoded_image
...
Please directly answer the question based on the input images and corresponding captions. \
Question: <question>. Output in the following format: {'Answer': 'Direct and Brief Answer to the Question'}. \n"

def infer_claude(qasa_data, args):
    ...
    for paper_id, paper in qasa_data.items():
        ...
        for question_idx, question in enumerate(paper['question']):
            ...
            answer, all_figures_captions, all_figures_encoded, referred_figures_indices, all_figures_modified, referred_figures = prepare_inputs(paper, question_idx)
            ...
            input_prompt[0]['content'].append({
                "type": "text",
                "text": _PROMPT.replace('<question>', question)
            })

            for idx, figure_bytes in enumerate(list(all_figures_encoded.keys())):
                input_prompt[0]['content'].append({"type": "text", "text": "Image {}: ".format(idx)})
                input_prompt[0]['content'].append({"type": "image",
                    "source": {
                        "type": "base64",
                        "media_type": "image/png",
                        "data": all_figures_encoded[figure_bytes],
                    }})
                input_prompt[0]['content'].append({"type": "text", "text": "Caption {}: {} \n\n".format(idx, all_figures_captions[idx])})
