...
def prepare_inputs(paper, question_idx):
  ...
  for idx, figure in enumerate(all_figures_modified):
    all_figures_bytes['figure_{}'.format(idx)] = {
      'mime_type': 'image/png',
      'data': cv2_imagebytes(os.path.join(_QASPER_IMAGE_ROOT, paper['arxiv_id'], figure))
    }
  
  para_list = ['\n'.join(para['paragraphs']) for para in paper['full_text']]
  full_text = '\n\n'.join(para_list)

  return answer, all_figures_captions, all_figures_bytes, referred_figures_indices, all_figures_modified, referred_figures, full_text
    ...
    for paper_id, paper in qasper_data.items():
        ...
        try:
          for question_idx, question in enumerate(paper['question']):
              ...
              answer, all_figures_captions, all_figures_bytes, referred_figures_indices, all_figures_modified, referred_figures, full_text = prepare_inputs(paper, question_idx)

              contents = [_PROMPT.replace('<question>', question)]

              contents.append("Paragraphs from the paper: {}: ".format(full_text))
